...
// This can be used for the singleplayer mode(s) of Tetris
type Game struct {
	matrix           tetrislogic.Matrix     // The Matrix of cells on which the game is played
	nextQueue        *tetrislogic.NextQueue // The queue of upcoming Tetriminos
	tetInPlay        *tetrislogic.Tetrimino // The current Tetrimino in play
	ghostTet         *tetrislogic.Tetrimino // The ghost Tetrimino
	holdQueue        *tetrislogic.Tetrimino // The Tetrimino that is being held
	...
	scoring          *tetrislogic.Scoring   // The scoring system
	fall             *tetrislogic.Fall      // The system for calculating the fall speed
}
...
func NewGame(in *Input) (*Game, error) {
	matrix, err := tetrislogic.NewMatrix(40, 10)
	if err != nil {
		return nil, err
	}
	nq := tetrislogic.NewNextQueue(matrix.GetSkyline(), tetrislogic.WithRandSource(in.Rand))

	scoring, err := tetrislogic.NewScoring(
	...
	g := &Game{
		...
		nextQueue:        nq,
		tetInPlay:        nq.Next(),
		holdQueue:        tetrislogic.GetEmptyTetrimino(),
		gameOver:         false,
		softDropStartRow: matrix.GetHeight(),
		scoring:          scoring,
		fall:             tetrislogic.NewFall(in.Level),
	}
...
func (g *Game) Hold() (bool, error) {
	...
	t, err := tetrislogic.GetTetrimino(g.holdQueue.Value)
	if err != nil {
		return false, err
	}
